...
from utils import *
from memory import AnchorReservoir, CacheMemory, Mamba3Memory, SSMemory, TCAReservoirMemory
...
def get_arguments():
    ...
    parser.add_argument('--enable-token-condensation', dest='enable_token_condensation', action='store_true', help='Enable official-style TCA token condensation for ViT backbones.')
    parser.add_argument('--token-condensation-mode', dest='token_condensation_mode', type=str, choices=['tca-ours'], default='tca-ours', help='Token condensation mode. Default: tca-ours.')
    parser.add_argument('--tca-drop-rate', dest='tca_drop_rate', type=float, default=0.035, help='Official-style TCA drop rate used at layers 3, 6, 9.')
    parser.add_argument('--tca-drop-locations', dest='tca_drop_locations', type=str, default='3,6,9', help='Comma-separated ViT layer indices where TCA drop is applied.')
    parser.add_argument('--tca-reservoir-size', dest='tca_reservoir_size', type=int, default=3, help='Per-class TCA reservoir size.')
    parser.add_argument('--tca-scale', dest='tca_scale', type=float, default=5.5, help='Layer scaling temperature for TCA reservoir logits.')
    parser.add_argument('--tca-lambda', dest='tca_lambda', type=float, default=2.0, help='Strength of TCA reservoir logits.')
...
def run_test_tda(
    ...
    if hasattr(clip_model, 'update_tca_cls_token_cache'):
        clip_model.update_tca_cls_token_cache(None)
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(tqdm(loader, desc='Processed test images: ')):
            ...
            if use_consensus:
                ...
                token_condense_config = None
                if token_condense_enabled:
                    token_condense_config = {
                        'enabled': True,
                        'mode': token_condense_kwargs['mode'],
                        'drop_rate': token_condense_kwargs['drop_rate'],
                        'drop_locations': token_condense_kwargs['drop_locations'],
                    }
            ...
            token_condense_details = visual_details.get('token_condense')
            ...
            if token_condense_details is None:
                ...
                token_condense_merged_tokens_history.append(0.0)
                ...
                token_condense_apply_history.append(1.0 if token_condense_details.get('applied_any', False) else 0.0)
                kept_counts = token_condense_details.get('kept_token_counts', [])
                merged_counts = token_condense_details.get('merged_token_counts', [])
        ...
        if tca_memory is not None:
            adapter_memory_bytes += tca_memory.memory_bytes()
        ...
        if len(token_condense_attempt_history) > 0:
            ...
            tc_merged_tokens_prefix = np.cumsum(np.array(token_condense_merged_tokens_history, dtype=np.float64))
            ...
            details['avg_token_condense_merged_tokens'] = float(tc_merged_tokens_prefix[-1] / tc_steps[-1])
        ...
        if tca_memory is not None:
            tca_stats = tca_memory.stats()
            details['tca_update_accept_count'] = tca_stats['accepted_updates']
            details['tca_update_attempt_count'] = tca_stats['total_update_attempts']
            details['tca_update_accept_rate'] = tca_stats['accept_rate']
            details['tca_fill_ratio'] = tca_stats['fill_ratio']
            if len(tca_fill_ratio_history) > 0:
                tca_fill_prefix = np.cumsum(np.array(tca_fill_ratio_history, dtype=np.float64))
...
def main():
    ...
    token_condense_kwargs = {
        ...
        'drop_locations': _parse_int_list(args.tca_drop_locations),
